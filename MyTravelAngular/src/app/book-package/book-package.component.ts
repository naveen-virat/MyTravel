...
export class BookPackageComponent implements OnInit {
  ...
  submitBookForm(form: NgForm) {
    ...
    var bookpkg: IBookPackage = {
      contactNumber: parseInt(form.value.contactNumber), emailId: email, address: form.value.address, dateOfTravel: form.value.dateOfTravel, numberOfAdults: parseInt(form.value.adults),
