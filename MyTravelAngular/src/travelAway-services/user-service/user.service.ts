...
export class UserService {
  ...
  validateCredentials(email: string, password: string,loginRole: number): Observable<number> {
    ...
    if (loginRole == 1) {
      ...
      //custObj = { EmailId: email, UserPassword: password, FirstName: "", LastName: "", RoleId: 1, Gender: "", DateOfBirth: new Date, Address: "", ContactNumber: 9999999999 };
      let url: string = 'https://localhost:44393/api/TravelAway/ValidateLoginCustomer?emailId=' + email + '&password='+password
      temp = this.http.get<number>(url).pipe(catchError(this.errorHandler))
    } else if (loginRole == 2) {
      var EmpObj: IEmployee;
      EmpObj = { EmailId: email, Password: password, FirstName: "", LastName: "", RoleId: 0 };
      let url: string = 'https://localhost:44393/api/TravelAway/ValidateLoginEmployee?emailId=' + email + '&password=' + password
      temp = this.http.get<number>(url).pipe(catchError(this.errorHandler))
    }
    return temp;
  }
